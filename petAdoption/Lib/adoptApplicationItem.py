...
from Lib.adoptApplicationWidget import Ui_Form
try:
    ...
    from PySide2.QtWidgets import (QApplication, QLayout, QPushButton,
                                   QSizePolicy, QWidget)
class adoptApplicationItem(QWidget, Ui_Form):

    def __init__(self, *args):
        super(adoptApplicationItem, self).__init__(*args)
        self.setupUi(self)
        self.pushButton.clicked.connect(lambda : self.click_drop())
    ...
    #   self.label_22.setText("<html><head/><body><p><span style=\" color:#222222;\">"+str(a_result)+ "</span></p></body></html>")

    def click_drop(self):
        self.conn = pymysql.connect(host='localhost', port=3306, user='root', password="123123",
                                    db="petadoption")
        self.cur = self.conn.cursor()
        sql_adopt_drop = "DELETE FROM petadoption.adoptionapplication where ap_id='{}';".format(self.ap_id)
        self.cur.execute(sql_adopt_drop)
        self.conn.commit()
        self.cur.close()
        self.conn.close()
        self.close()